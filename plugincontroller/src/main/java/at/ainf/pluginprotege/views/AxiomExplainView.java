...
public class AxiomExplainView extends AbstractOWLViewComponent implements AxiomChangedListener, ResetReqListener {
    ...
    protected WorkspaceTab getWS() {
        return ((WorkspaceTab) getOWLWorkspace().getWorkspaceTab("at.ainf.pluginprotege.WorkspaceTab"));
    }
    ...
    protected void initialiseOWLView() throws Exception {
        ...
        if (DebugManager.getInstance().getAx() != null)
            ...
            // ((WorkspaceTab) getOWLWorkspace().getWorkspaceTab("at.ainf.pluginprotege.WorkspaceTab")).setExpl(this);
    ...
    public void processAxiom (OWLLogicalAxiom axiom) {
        ...
        if (getWS().getSearch() instanceof UniformCostSearch) {
            es = ((UniformCostSearch<OWLLogicalAxiom>) getWS().getSearch()).getNodeCostsEstimator();
        }
