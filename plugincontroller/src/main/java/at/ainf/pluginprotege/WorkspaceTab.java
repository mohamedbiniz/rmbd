...
import at.ainf.pluginprotege.configwizard.DebuggerWizard;
...
import at.ainf.pluginprotege.testcasesentailmentsview.TcaeFrameSection;
...
public class
    ...
    public void loadTestcasesAction() {
        try {
            ...
            OWLOntology orig = getOwlTheory().getOriginalOntology();
    ...
    public void saveProbabilites(HashMap<ManchesterOWLSyntax, Double> map, ManchesterOWLSyntax[] keywords) {
            ...
            OWLOntologyManager manager = getOwlTheory().getOntology().getOWLOntologyManager();
            ...
            for (Iterator<OWLAnnotation> iter = getOwlTheory().getOriginalOntology().getAnnotations().iterator(); iter.hasNext(); ) {
                OWLAnnotation ann = iter.next();
                if (ann.getValue() instanceof OWLLiteral) {
                    String s = ((OWLLiteral) ann.getValue()).getLiteral();
                    if (s.startsWith("Probabilities ")) {
                        manager.applyChange(new RemoveOntologyAnnotation(getOwlTheory().getOriginalOntology(), ann));
                    }
            ...
            manager.applyChange(new AddOntologyAnnotation(getOwlTheory().getOriginalOntology(), anno));
            getOWLModelManager().setDirty(getOwlTheory().getOriginalOntology());
    ...
    public void createOWLTheory() {
        ...
        if (theory != null) {
            theory.getOntology().getOWLOntologyManager().removeOntology(theory.getOntology());
            getOWLModelManager().removeOntology(theory.getOntology());
        }
        ...
        try {
            ...
            /*if (theory != null) {
                ...
                theory = new OWLTheory(reasonerFactory, ontology, bax, QueryDebuggerPreference.getInstance().isTestIncoherencyInconsistency());
                theory.setIncludeSubClassOfAxioms(QueryDebuggerPreference.getInstance().isSubClassOfActivated());
                theory.setIncludeClassAssertionAxioms(QueryDebuggerPreference.getInstance().isClassAssertionActivated());
                theory.setIncludeEquivalentClassAxioms(QueryDebuggerPreference.getInstance().isEquivalentClassActivated());
                theory.setIncludeDisjointClassAxioms(QueryDebuggerPreference.getInstance().isDisjointClassesActivated());
                theory.setIncludePropertyAssertAxioms(QueryDebuggerPreference.getInstance().isPropertyAssertActivated());
                theory.setIncludeReferencingThingAxioms(QueryDebuggerPreference.getInstance().isIncludeTrivialAxiomsActivated());
                theory.setIncludeOntologyAxioms(QueryDebuggerPreference.getInstance().isIncludeOntologyAxiomsActivated());
                /* getOWLModelManager().getDirtyOntologies().remove(theory.getOntology());
            }*/
