...
public class DiagnosisTests {
    ...
    private boolean test(File file, int number) throws URISyntaxException, SolverException, OWLException, UnsatisfiableFormulasException {
        ...
        start();
        if (debugger.debug()) {
            logger.info(file.getName() + " - with " + debugger.getConflicts().size() + " minimal conflict(s) and " + debugger.getHittingSets().size() + " hitting set(s)");
            logResult(debugger.getHittingSets(), "Hitting set cardinalities: ");
